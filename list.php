...
if ($result5 = mysqli_query($link, $query5)) {
	while ($row5 = mysqli_fetch_row($result5)) {
		...
		$query17 = "SELECT datumUdalosti FROM datove_vety WHERE idUdalost = '$idUdalost' AND datumUdalosti IS NOT NULL AND datumUdalosti != '' ORDER BY datumVytvoreni LIMIT 1;";
		...
		$query59 = "SELECT humanID FROM datove_vety WHERE idUdalost = '$idUdalost' ORDER BY datumVytvoreni LIMIT 1;;";
		...
		$slozka =  substr($human_id, 7, 1);
		$kraj = substr($human_id, 8, 3);
...
//		echo "<tr><td>$idUdalost</td><td>$casPrichoduHovoruform</td><td>$datumUdalostiform</td><td>$datumUlozeniform</td><td>$popis</td><td>$hzsform</td><td>$pcrform</td><td>$zzsdform</td><td>$slozka</td><td>$kraj</td><td>$typUdalosti</td><td>$podtyp</td></tr>";
		...
		$query107 = "INSERT INTO udalosti (idUdalost, casPrichoduHovoru, datumUdalosti, datumUlozeni, popis, hzs, pcr, zzs, slozka, kraj, typ, podtyp, humanID) VALUES ('$idUdalost','$casPrichoduHovoru','$datumUdalosti','$datumUlozeni','$popis','$hzs','$pcr','$zzs','$slozka','$kraj','$typUdalosti','$podtyp', '$human_id');";
